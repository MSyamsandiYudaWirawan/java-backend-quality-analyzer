...
- **Load scenarios are agent-authored — and authoring is non-deterministic, so the artifact is the contract, not the agent.** Regenerating a k6 script for the same repo can legitimately yield a different scenario (different endpoints, payloads, id handling); if re-runs regenerated, two "reproductions" could silently measure different things. Instead, scripts are generated from the target's API surface against a fixed scenario standard (mixed create→read, fixed VU/duration profile across all repos), smoke-validated, and committed as evidence (`evidence/advanced/h2/<repo>/{slots.json,load-test.js}`). Re-runs always use the committed artifact — measurements are reproducible even though authoring is agentic. What still varies run-to-run is measurement noise, bounded by the fixed docker envelope (identical CPU/memory limits for every repo).
